...
CC ?= cc
USERFLAGS+=
override CFLAGS+=-I$(IDIR) -g -Wall -Wpedantic $(USERFLAGS) -std=c11 -Wno-format-extra-args `pkg-config --cflags gtk+-3.0` `pkg-config --libs gtk+-3.0`
...
ijvm-gui: src/machine.c src/stack.c gui/gui.c gui/main.c
	echo $(SRCS)
	$(CC) -o $@ $^ $(CFLAGS) $(LIBS)
...
dist: clean
	tar -cvzf dist.tar.gz src/*.c src/*.h include/*.h gui/*.c Makefile README.md
